...
- **비교 및 US양식 다운로드**: 변경 내용(Track Changes)을 유지한 채 US 특허출원 양식(A4, Arial 12pt, 줄간격 2줄, 5행마다 줄번호, 페이지번호, SEQ 필드 단락번호, 청구항 들여쓰기)을 적용해 다운로드. 양식표준화가 자동 적용되고(멱등 — 이미 표준화된 문서에는 중복 적용되지 않음) 기존 `[0001]` 텍스트 단락번호는 SEQ 필드로 대체되며(양쪽 문서 공통 전처리라 변경추적에 표시되지 않음), 삽입/삭제 단락의 SEQ 번호도 개정(ins/del)으로 감싸 변경 수락·거부와 함께 정리됨
...
- **US양식 줄간격 2줄**: 미국 특허출원 규정(37 CFR 1.52(b)(2)(ii))의 배행 요건에 맞춰 Word 단락
  대화상자에 **줄간격 `2줄`**로 표시되도록 `w:line="480"`(= 240 × 2) + `w:lineRule="auto"`를 쓴다.
  종전에는 고정 행 높이 547 DXA를 써서 대화상자에 **`고정 27.35pt`**로 표시됐다.
  줄 눈금(`docGrid w:type="lines"`)은 배수 줄간격 한 행이 눈금 두 칸을 차지해 행 간격이 두 배로
  벌어지므로 걸지 않고, Word 기본값 `docGrid w:linePitch="360"`을 쓴다.
  배수 2.0이면 Arial 12pt 한 행이 약 552 DXA(27.6pt)여서 본문영역
  `16838 - 1440 - 1701 = 13697` DXA에는 **24행**이 들어간다(A4 및 여백 등 나머지 양식은 그대로 유지)
...
- 머리글 단락에 **단락 뒤 공백 8pt**(Word의 `선 및 단락 간격 > 단락 뒤에 공백 추가`와 동일)를 적용해
  본문과 간격을 둔다. 행 높이는 `exact`로 고정해 머리글 높이를 360 DXA(행 200 + 뒤 공백 160)로
  확정하고, US양식의 머리글 여백을 1000으로 잡아 `1000 + 360 = 1360 ≤ 위 여백 1440` 이 되도록 했다.
  Word는 본문 시작 위치를 `max(위 여백, 머리글 여백 + 머리글 높이)`로 잡으므로, 이렇게 해야
  **머리글을 켜도 본문이 밀리지 않아 페이지당 행수가 유지**된다
- 적용 대상: 탭1 KIPO 출원서식/ROPKS, 탭2 워드파일/US양식, 탭3 영문본/국문본/한영혼합본/병합결과,
  탭4 텍스트 비교/DOCX 비교/비교 US양식, 탭5 DOCX 저장
- 비어 있던 머리글은 덧붙이지 않고 대체하는 방식이라 **US양식의 페이지당 행수가 그대로 유지**됨
