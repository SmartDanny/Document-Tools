...
- **비교 및 US양식 다운로드**: 변경 내용(Track Changes)을 유지한 채 US 특허출원 양식(A4, Arial 12pt, 25행/페이지 고정 행 높이 547 DXA, 5행마다 줄번호, 페이지번호, SEQ 필드 단락번호, 청구항 들여쓰기)을 적용해 다운로드. 양식표준화가 자동 적용되고(멱등 — 이미 표준화된 문서에는 중복 적용되지 않음) 기존 `[0001]` 텍스트 단락번호는 SEQ 필드로 대체되며(양쪽 문서 공통 전처리라 변경추적에 표시되지 않음), 삽입/삭제 단락의 SEQ 번호도 개정(ins/del)으로 감싸 변경 수락·거부와 함께 정리됨
...
- **US양식 25행/페이지**: 본문영역 `16838 - 1440 - 1701 = 13697` DXA에 25행이 들어가려면 행 높이가
  `13697 / 25 = 547.88` **이하**여야 한다. 종전 값 548은 25행 합계가 13700으로 본문영역을 3 DXA 넘겨
  실제로는 **24행만** 표시됐다. 547로 내려 25행 합계 13675(여유 22 DXA)가 되어 줄번호가 25까지 현출된다
...
- 머리글 단락에 **단락 뒤 공백 8pt**(Word의 `선 및 단락 간격 > 단락 뒤에 공백 추가`와 동일)를 적용해
  ...
  **머리글을 켜도 본문이 밀리지 않아 25행/페이지가 유지**된다
