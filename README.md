...
- 비교 결과 DOCX 다운로드
- **비교 및 US양식 다운로드**: 변경 내용(Track Changes)을 유지한 채 US 특허출원 양식(A4, Arial 12pt, 25행/페이지 고정 행 높이, 5행마다 줄번호, 페이지번호, SEQ 필드 단락번호)을 적용해 다운로드. 양식표준화가 자동 적용되고 기존 `[0001]` 텍스트 단락번호는 SEQ 필드로 대체되며(양쪽 문서 공통 전처리라 변경추적에 표시되지 않음), 삽입/삭제 단락의 SEQ 번호도 개정(ins/del)으로 감싸 변경 수락·거부와 함께 정리됨
