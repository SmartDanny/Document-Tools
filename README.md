...
- **`.fin`(KIPO 전자출원 파일) 지원**: `.fin`(zip → `.hlz` → KIPO KEAPS XML + 도면)을 분석해
  - **KIPO 출원서식 DOCX**로 변환 — 실제 KIPO 서식 역설계 반영: 명세서/청구범위/요약서/도면 4부(部) 구조(부 사이 페이지 나누기), 중앙 볼드 부 헤더, 국문 【】 부제, 제목 국문/영문 분리, `[NNNN]` 단락번호, 도면 이미지 임베드 후 `[도 N]` 캡션, Malgun Gothic 10pt. 파일명: `<원본파일명>_출원명세서.docx`
  - **해외출원용 국문(ROPKS) DOCX**로 변환 — ROPKS 샘플 역설계 반영: 사무소표준US 부제(볼드+밑줄), 바탕체 12pt, 행간(line 518 auto), 본문 첫줄 들여쓰기·**양쪽맞춤**, **줄번호(페이지마다 1부터)**, 지정 여백, 청구범위·요약서·도면(및 각 도면)의 **페이지 나누기**, 유니코드 첨자 자동 정규화. 파일명은 **해외관리번호** 입력에 따라 자동 지정 — `OPP20******US` 입력 시 국가코드 US를 떼고 `OPP20******ROPKS_(오늘6자리)`, 미입력 시 `ROPKS_(오늘6자리)`.docx
